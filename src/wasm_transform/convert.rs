...
use crate::wasm_transform::Error;
...
pub(super) mod parser_to_internal {
    ...
    fn data_kind(kind: wasmparser::DataKind) -> Result<crate::wasm_transform::DataSegmentKind> {
        Ok(match kind {
            wasmparser::DataKind::Passive => crate::wasm_transform::DataSegmentKind::Passive,
            wasmparser::DataKind::Active {
                memory_index,
                offset_expr,
            } => crate::wasm_transform::DataSegmentKind::Active {
                memory_index,
    ...
    ) -> Result<crate::wasm_transform::DataSegment> {
        Ok(crate::wasm_transform::DataSegment {
    ...
    ) -> Result<crate::wasm_transform::ElementKind> {
        match kind {
            wasmparser::ElementKind::Passive => Ok(crate::wasm_transform::ElementKind::Passive),
            wasmparser::ElementKind::Declared => Ok(crate::wasm_transform::ElementKind::Declared),
            wasmparser::ElementKind::Active {
                table_index,
                offset_expr,
            } => Ok(crate::wasm_transform::ElementKind::Active {
                table_index,
                offset_expr: const_expr(offset_expr)?,
            }),
        }
    ...
    ) -> Result<crate::wasm_transform::ElementItems> {
        match items {
            wasmparser::ElementItems::Functions(reader) => {
                let functions = reader
                    .into_iter()
                    .collect::<std::result::Result<Vec<_>, _>>()?;
                Ok(crate::wasm_transform::ElementItems::Functions(functions))
            }
            wasmparser::ElementItems::Expressions(ref_type, reader) => {
                let exprs = reader
                    ...
                    .collect::<std::result::Result<Vec<_>, _>>()?;
                Ok(crate::wasm_transform::ElementItems::ConstExprs {
                    ty: ref_type,
                    exprs,
                })
            }
    ...
    pub(crate) fn global(global: wasmparser::Global) -> Result<crate::wasm_transform::Global> {
        Ok(crate::wasm_transform::Global {
            ty: global.ty,
...
pub(super) mod internal_to_encoder {
    ...
    pub(crate) fn op(op: wasmparser::Operator<'_>) -> Result<wasm_encoder::Instruction<'static>> {
        ...
        macro_rules! convert {
            ...
            (map $arg:ident tys) => (
                Cow::Owned($arg.into_iter().map(|ty| {
                    wasm_encoder::ValType::try_from(ty)
                        .map_err(|_err| Error::ConversionError(format!("Failed to convert type: {:?}", ty)))
                }).collect::<std::result::Result<std::vec::Vec<_>, Error>>()?)
            );
            (map $arg:ident hty) => (
            ...
            (build F32Const $arg:ident) => (Ok(I::F32Const(wasm_encoder::Ieee32::from($arg))));
            (build F64Const $arg:ident) => (Ok(I::F64Const(wasm_encoder::Ieee64::from($arg))));
